...
class nnUNetWithClassificationLogger(nnUNetLogger):
    ...
    def log(self, key, value, epoch: int):
        ...
        if key == 'classification_f1':
            new_ema_f1 = self.my_fantastic_logging['ema_f1'][epoch - 1] * 0.9 + 0.1 * value \
                if len(self.my_fantastic_logging['ema_f1']) > 0 else value
            self.log('ema_f1', new_ema_f1, epoch)

    def __init__(self, verbose: bool = False):
        self.my_fantastic_logging = {
            'mean_fg_dice': list(),
            'classification_f1': list(),
            'ema_fg_dice': list(),
            'ema_f1': list(),
            'dice_per_class_or_region': list(),
            'train_losses': list(),
    ...
    def plot_progress_png(self, output_folder):
        # we infer the epoch form our internal logging
        epoch = len(self.my_fantastic_logging["lrs"]) - 1  # lists of epoch 0 have len 1
        sns.set(font_scale=2.5)
        fig, ax_all = plt.subplots(3, 1, figsize=(30, 54))
        # regular progress.png as we are used to from previous nnU-Net versions
        ax = ax_all[0]
        ...
        ax.plot(x_values, self.my_fantastic_logging['train_losses'][:epoch + 1], color='b', ls='-', label="loss_tr", linewidth=4)
        ax.plot(x_values, self.my_fantastic_logging['val_losses'][:epoch + 1], color='r', ls='-', label="loss_val", linewidth=4)
        ax2.plot(x_values, self.my_fantastic_logging['ema_f1'][:epoch + 1], color='purple', ls='dotted', label="Classification F1",
                 linewidth=3)
