...
class OrionDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        if case_identifiers is None:
            case_identifiers = self.generate_case_identifiers(100)
    ...
    def generate_recording(self, length, n_classes):
        sig = np.random.rand(length)
        seg = np.random.randint(0, n_classes, length)
        return sig, seg
    
    def generate_case_identifiers(self, n_cases):
        return [f'case_{i}' for i in range(n_cases)]
